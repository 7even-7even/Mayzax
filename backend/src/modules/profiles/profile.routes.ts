...
router.post('/', requireRole(Role.ADMIN, Role.RECRUITER), validate({ body: createProfileSchema }), profileController.createProfile);
router.patch('/:id', requireRole(Role.ADMIN, Role.RECRUITER), validate({ params: idParamSchema, body: updateProfileSchema }), profileController.updateProfile);
router.patch(
  '/:id/assign',
