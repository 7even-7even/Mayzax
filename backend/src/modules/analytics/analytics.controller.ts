...
export const getDashboardOverview = asyncHandler(async (req: Request, res: Response) => {
  const result = await analyticsService.getDashboardOverview(req.query as any, { id: req.user!.sub, role: req.user!.role });
...
export const getRecruiterBreakdown = asyncHandler(async (req: Request, res: Response) => {
  const result = await analyticsService.getRecruiterBreakdown(req.params.id, { id: req.user!.sub, role: req.user!.role });
  res.status(200).json({ success: true, data: result });
});

export const getDailyCounts = asyncHandler(async (req: Request, res: Response) => {
  const result = await analyticsService.getDailyCounts(req.query as any, { id: req.user!.sub, role: req.user!.role });
  res.status(200).json({ success: true, data: result });
});
...
export const getGlobalSummary = asyncHandler(async (req: Request, res: Response) => {
  const result = await analyticsService.getGlobalSummary({ id: req.user!.sub, role: req.user!.role });
  res.status(200).json({ success: true, data: result });
});
