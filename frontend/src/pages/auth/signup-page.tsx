        ...
        <div className="relative z-10 flex flex-col gap-8">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.6 }}>
            <h1 className="mt-6 text-5xl font-bold tracking-tight text-white leading-[1.05]">
              Empower your
              <br />
              <span className="bg-gradient-to-r from-white via-white to-white/70 bg-clip-text text-transparent">recruiting pipeline</span>
            </h1>
          </motion.div>

          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }} className="grid grid-cols-3 gap-3 max-w-lg">
            {[
              { icon: ShieldCheck, label: 'Verified Team', sub: 'Scoped Roles' },
              { icon: Zap, label: 'Fast Shift Log', sub: 'Instant Sync' },
              { icon: Users, label: 'Collaboration', sub: 'Multi-assignment' },
            ].map((f, i) => (
      ...
      <div className="relative flex w-full flex-col items-center justify-center px-4 py-8 lg:w-[42%] bg-white dark:bg-slate-950 lg:shadow-[-20px_0_80px_rgba(42,93,168,0.08)] overflow-hidden" onMouseMove={handleMouseMove}>
        ...
        <div className="relative z-10 w-full max-w-[380px]">
          <div className="relative overflow-hidden rounded-[24px] border border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 p-8 shadow-[0_20px_80px_rgba(42,93,168,0.08),0_0_0_1px_rgba(0,0,0,0.03)] dark:shadow-2xl">
            ...
            <div className="relative">
              <div className="mb-7">
                <div className="inline-flex items-center gap-2 rounded-full bg-mayzax-blue-50 dark:bg-mayzax-blue-950/50 border border-mayzax-blue-100 dark:border-mayzax-blue-800 px-3 py-1 text-[11px] font-bold tracking-wide text-mayzax-blue-700 dark:text-mayzax-blue-300">
                  <span className="h-1.5 w-1.5 rounded-full bg-mayzax-green-500 animate-pulse" />
                  RECRUITER ONBOARDING
                </div>
                <h2 className="mt-4 text-[22px] font-bold tracking-tight text-slate-900 dark:text-white">Create your account</h2>
                <p className="mt-1.5 text-[13px] leading-relaxed text-slate-500 dark:text-slate-400">Join Mayzax ATS • Instant access • No spam</p>
              </div>

              <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" noValidate>
                <div className="space-y-1.5">
                  <Label className="text-xs font-bold tracking-wide uppercase text-slate-700 dark:text-slate-300">Full Name</Label>
                  <div className="relative group">
                    ...
                    <Input placeholder="Your full name" className="h-[46px] pl-10 rounded-xl border-slate-200 dark:border-slate-700 bg-slate-50/70 dark:bg-slate-800/80 focus:bg-white dark:focus:bg-slate-800 focus:border-mayzax-blue-300 focus:ring-4 focus:ring-mayzax-blue-50/20 text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 text-[14px] transition-all" {...register('name')} />
                  </div>
                  {errors.name && <p className="text-xs text-red-500">{errors.name.message}</p>}
                </div>

                <div className="space-y-1.5">
                  <Label className="text-xs font-bold tracking-wide uppercase text-slate-700 dark:text-slate-300">Work Email</Label>
                  <div className="relative group">
                    ...
                    <Input type="email" placeholder="you@company.com" className="h-[46px] pl-10 rounded-xl border-slate-200 dark:border-slate-700 bg-slate-50/70 dark:bg-slate-800/80 focus:bg-white dark:focus:bg-slate-800 focus:border-mayzax-blue-300 focus:ring-4 focus:ring-mayzax-blue-50/20 text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 text-[14px] transition-all" {...register('email')} />
                  </div>
                  {errors.email && <p className="text-xs text-red-500">{errors.email.message}</p>}
                </div>

                <div className="space-y-1.5">
                  <Label className="text-xs font-bold tracking-wide uppercase text-slate-700 dark:text-slate-300">Password</Label>
                  <div className="relative group">
                    ...
                    <Input type={showPassword ? 'text' : 'password'} placeholder="Create a strong password" className="h-[46px] pl-10 pr-10 rounded-xl border-slate-200 dark:border-slate-700 bg-slate-50/70 dark:bg-slate-800/80 focus:bg-white dark:focus:bg-slate-800 focus:border-mayzax-blue-300 focus:ring-4 focus:ring-mayzax-blue-50/20 text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 text-[14px] transition-all" {...register('password')} />
                    <button type="button" onClick={() => setShowPassword((v) => !v)} className="absolute right-3 top-1/2 -translate-y-1/2 flex h-7 w-7 items-center justify-center rounded-full hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-400" tabIndex={-1}>
                ...
                <div className="space-y-1.5">
                  <Label className="text-xs font-bold tracking-wide uppercase text-slate-700 dark:text-slate-300">Confirm Password</Label>
                  <div className="relative group">
                    <Lock className="pointer-events-none absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400 group-focus-within:text-mayzax-blue-500 transition-colors" />
                    <Input type={showPassword ? 'text' : 'password'} placeholder="Repeat password" className="h-[46px] pl-10 rounded-xl border-slate-200 dark:border-slate-700 bg-slate-50/70 dark:bg-slate-800/80 focus:bg-white dark:focus:bg-slate-800 focus:border-mayzax-blue-300 focus:ring-4 focus:ring-mayzax-blue-50/20 text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 text-[14px] transition-all" {...register('confirmPassword')} />
                  </div>
              ...
              <p className="mt-6 text-center text-sm text-slate-500 dark:text-slate-400">
                ...
                <Link to="/login" className="font-bold text-mayzax-blue-750 dark:text-mayzax-blue-400 hover:text-mayzax-blue-800 dark:hover:text-mayzax-blue-300 transition-colors inline-flex items-center gap-1">
