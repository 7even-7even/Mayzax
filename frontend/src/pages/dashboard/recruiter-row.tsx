  ...
  return (
    <>
      <MotionRow
        ...
        className="group cursor-pointer border-b border-slate-100 transition-all hover:bg-gradient-to-r hover:from-indigo-50/40 hover:to-violet-50/40 hover:shadow-sm"
        onClick={onToggle}
      >
        <TableCell>
          <div className="flex items-center gap-3">
            <motion.span animate={{ rotate: expanded ? 90 : 0 }} transition={{ duration: 0.2 }} className="flex h-6 w-6 items-center justify-center rounded-full bg-slate-100 group-hover:bg-slate-900 group-hover:text-white transition-colors">
        ...
        <TableCell>
          <Badge className={`${row.isActive ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-slate-100 text-slate-500 border-slate-200'} border rounded-full px-2.5 py-1 text-xs font-medium`}>
        ...
        <TableCell className="text-sm font-semibold text-slate-700">
          <span className="inline-flex items-center rounded-full bg-slate-100 px-2.5 py-1 text-xs font-bold">{row.assignedProfiles}</span>
        </TableCell>
      ...
      <AnimatePresence initial={false}>
        {expanded && (
          <TableRow className="bg-gradient-to-br from-slate-50/80 to-indigo-50/20 hover:from-slate-50/80 hover:to-indigo-50/20">
                    ...
                    <div className="grid grid-cols-1 gap-2.5 sm:grid-cols-2 lg:grid-cols-3">
                      {data.profileWiseCounts.map((p, i) => (
                        <motion.div
                          ...
                          className="group relative overflow-hidden rounded-xl border border-slate-200 bg-white p-3 shadow-sm hover:shadow-md hover:border-violet-200 transition-all"
                        >
                          ...
                          <div className="flex items-start justify-between gap-2">
                            <div className="min-w-0">
                              <p className="text-sm font-semibold text-slate-800 truncate">{p.candidateName}</p>
                              {p.technology && <p className="text-[11px] text-slate-400 mt-0.5">{p.technology}</p>}
                            </div>
